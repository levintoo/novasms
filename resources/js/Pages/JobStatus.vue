...
let update_interval
onMounted(() => {
    update_interval = setInterval(update,3000)
        ...
        <template #tbody>
            <TableBody v-if="jobs.data.length > 0">
                <TableBodyItem v-for="job in jobs.data">
                    ...
                    <TableData >
                        <span v-if="!job.info.cancelledAt && !job.info.finishedAt" class="bg-gray-100 text-gray-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full">
                           {{ !!job.info.finishedAt ?? '-' }}
                        </span>
                        <span v-else-if="!job.info.cancelledAt" class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full">
                           {{ !!job.info.finishedAt ?? '-' }}
                        </span>
