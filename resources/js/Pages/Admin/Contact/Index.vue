...
watch(params.value, debounce((params) => {
    router.get('/admin/contact/', { ...omitBy(params, v => v === "") }, { preserveScroll: true, replace: true, preserveState: true },)
},150));
...
const handleRestore = (id) => {
    ...
    router.patch('/admin/contact/restore/'+ id, {
        preserveScroll: true,
        onError: (errors) => {
            toast.add({
                message: 'something went wrong',
...
<template>
    ...
    <Table >
        <template #thead>
            <TableHead>
                <TableHeadItem :params="params" @click="sort('first_name')" sort="first_name" class="cursor-pointer" field="first name"/>
                <TableHeadItem :params="params" @click="sort('last_name')" sort="last_name" class="cursor-pointer" field="last name"/>
            ...
            <TableBody v-if="contacts.data.length > 0">
                <TableBodyItem v-for="contact in contacts.data">
                    ...
                    <TableData class="grid grid-cols-1 w-full">
                        ...
                        <span class="text-xs">{{ contact.user.email }}</span>
                    </TableData>
                    <TableData>
                        {{ contact.group ?? '-' }}
                    </TableData>
                    ...
                    <td class="text-center">
                        <IconButton v-if="!contact.trashed" @click="handleDelete(contact.id)"
                        ...
                        <IconButton v-if="contact.trashed" @click="handleRestore(contact.id)"
        ...
        <template #pagination>
            <Pagination v-if="contacts.links" :to="contacts.to" :from="contacts.from" :total="contacts.total" :links="contacts.links" />
        </template>
    </Table>
</template>
Admin
