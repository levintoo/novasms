...
watch(params.value, debounce((params) => {
    router.get(route('admin.contacts'), { ...omitBy(params, v => v === "") }, { preserveScroll: true, replace: true, preserveState: true },)
},150));
...
const handleRestore = (id) => {
    ...
    router.patch(route('admin.contact.restore',id), {
        preserveScroll: true,
        onError: (errors) => {
            toast.add({
                message: 'something went wrong',
...
<template>
    ...
    <Table >
        ...
        <template #pagination>
            <Pagination :to="contacts.to" :from="contacts.from" :total="contacts.total" :links="contacts.links" />
        </template>
    </Table>
</template>
